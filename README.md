...
## OminiX-SGLang Fork Status

This branch contains the headless OminiX scheduler boundary for running SGLang
below `OminiX-API`, without exposing SGLang's public OpenAI HTTP server in the
serving path.

Accepted DeepSeek-V4-Flash path:

```text
OminiX-API router-only
  -> sglang-ominix v0 shim --mode grpc
  -> SglangScheduler.Generate
  -> DeepSeek-V4-Flash CUDA runtime, TP=8 on 8x RTX 5090
```

Live CUDA acceptance passed on 2026-05-02 with functional, benchmark,
SM120 FlashMLA, safety-log, and cleanup gates all passing. The accepted report
is:
...
See [`docs/ominix/DEEPSEEK_V4_FLASH_FINAL_PATH.md`](docs/ominix/DEEPSEEK_V4_FLASH_FINAL_PATH.md)
for the final architecture, run command, acceptance gates, performance numbers,
